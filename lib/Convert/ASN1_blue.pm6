...
BEGIN {
  ...
  %EXPORT_TAGS = (
    ...
    tag   => [<asn_tag asn_decode_tag2 asn_decode_tag asn_encode_tag asn_decode_length asn_encode_length>]
  );
...
# $optn, $op, $stash, $var, $buf, $pos, $len
  ...
  if ($first +& 0x80) {
    ...
    my $mant = 0.0;
    for reverse unpack("C*",substr(@_[4],$estart+$expLen,@_[6]-1-$expLen)) <-> $_ {
