...
sub message {
  my $ldap = shift;
  my $shift = shift;
  # print "Shift hits the fan => $shift\n";
  if ($shift eq 'Net::LDAP::Bind') {
      # print "Bind that shift\n";
    'Net::LDAP::Bind'->new($ldap, @_);
    # $shift->new($ldap, @_);
  }
  else {
    $shift->new($ldap, @_);
...
sub search {
  ...
  dd \%stash;
  dd $control;
  print $stash{filter}{or}[0]{equalityMatch}{assertionValue}, "\n";
  $mesg->encode(
    searchRequest => \%stash,
    controls      => $control
  ) or return _error($ldap, $mesg, LDAP_ENCODING_ERROR, "$@");
...
sub process {
  ...
  for (my $ready = 1; $ready; $ready = $ldap->data_ready) {
    ...
    # print "ldap response pdu => ", $pdu, "\n";
    my $result = $LDAPResponse->decode($pdu)
      or return LDAP_DECODING_ERROR;
    # print "ldap response decoded => ", keys %{ $result }, "\n";
